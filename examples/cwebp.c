...
// TODO(skal): make this constant user-settable?
static const int32_t kAmp = 170;
...
static uint8_t GetImportance(const WebPPicture* const pic,
  ...
  // kAmp * (sum / area - 128) ~= (kAmp * sum) / area - kAmp * 128
  sum = kAmp * sum / area;
  lambda = 128 + (int)sum - 128 * (int)kAmp;
  lambda = (lambda < 0) ? 0 : (lambda > 255) ? 255 : lambda;
