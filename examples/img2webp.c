...
static void Help() {
  printf("Usage:\n\n");
  printf("  img2webp [general options] [image files...] [local options...]\n");
  printf("\n");

  printf("General options (only used during at the start of compression):\n");
  ...
  printf("Local options (only used starting at the next image input):\n");
  ...
  printf("example: img2webp -loop 2 in0.png -lossy in1.jpg\n"
         "                  -duration 80 in2.tiff -o out.webp\n");
}
...
static int SetLoopCount(int loop_count, WebPData* const webp_data) {
  ...
  WebPMuxError err = WebPMuxGetAnimationParams(mux, &new_params);
  ok = (err == WEBP_MUX_OK);
...
int main(int argc, char* argv[]) {
  ...
  // 1rst pass of option parsing
  for (int c = 1; ok && c < argc; ++c) {
    if (argv[c][0] == '-') {
      int parse_error = 0;
      ...
      ok = !parse_error;
      argv[c] = NULL;   // mark option as 'parsed' during 1rst pass
    } else {
      ++pic_num;
    }
  }
  ...
  for (int c = 1; ok && c < argc; ++c) {
    ...
    if (ok) {
      ...
      if (!ok) fprintf(stderr, "Invalid configuration.\n");
    }
 ...
 End:
  ...
  if (ok) {
    fprintf(stderr, "[%d frames, %ld bytes].\n", pic_num, webp_data.size);
  }
