...
static int ReadImage(const GCHAR filename[], WebPPicture* const pic) {
  ...
  // e.g., WebP.
  ok = ReadPictureWithWIC((const char*)filename, pic, 1, NULL);
  if (ok) return 1;
#endif
  if (!ImgIoUtilReadFile((const char*)filename, &data, &data_size)) return 0;
...
int MAIN(int argc, const GCHAR* argv[]) {
  ...
  int ok = ExUtilInitCommandLineArguments(argc - 1, (const char**)argv + 1,
                                          &cmd_args);
  if (!ok) return 1;
  argc = cmd_args.argc_;
  argv = (const GCHAR**)cmd_args.argv_;
  ...
  for (c = 0; ok && c < argc; ++c) {
    ...
    if (argv[c][0] == TO_GCHAR('-')) {    // parse local options
      int parse_error = 0;
    ...
    if (verbose) {
      fprintf(stderr, "Added frame #%3d at time %4d", pic_num, timestamp_ms);
 ...
 End:
  ...
  if (ok) {
    if (output != NULL) {
      ok = ImgIoUtilWriteFile((const char*)output, webp_data.bytes,
                              webp_data.size);
