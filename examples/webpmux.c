...
static int Process(const WebPMuxConfig* config) {
  ...
  switch (config->action_type_) {
    ...
    case ACTION_DURATION: {
      ...
      if (num_frames == 0) {
        printf("Doesn't look like the source is animated. "
        ...
        durations = (int*)calloc(num_frames, sizeof(*durations));
        if (durations == NULL) goto Err2;
        for (i = 0; i < feature->arg_count_; ++i) {
          int k;
          int args[3];
          int duration, start, end;
          int nb_args = ExUtilGetInts(feature->args_[i].params_, 10, 3, args);
          ok = (nb_args >= 1);
          if (!ok) goto Err3;
          duration = args[0];
          ...
          for (k = 1; k <= num_frames; ++k) {
            if (k >= start && k <= end) durations[k - 1] = duration;
          }
        }

        // Apply non-zero durations to their destination frames.
