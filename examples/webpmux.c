...
static void PrintHelp(void) {
  ...
  printf("\nNote: if a single file name is passed as argument, the arguments "
         "will be\n");
...
// Read a (text) file as a null-terminated C-string.
static char* ReadFileString(const char* const filename, size_t* size) {
  char* data = NULL;
  ...
  *size = ftell(in);
  fseek(in, 0, SEEK_SET);
  data = (char*)malloc((*size) + 1);   // with \0 terminator
  if (data == NULL) {
 Error:
    fclose(in);
    free(data);
  ...
  if (fread(data, *size, 1, in) != 1) goto Error;
  fclose(in);
  data[*size] = 0;
  return data;
}
...
static int SetArguments(int argc, const char* argv[],
  ...
  if (argc == 1 && argv[0][0] != '-') {
    char* cur;
    const char sep[] = " \t\r\n\f\v";
    config->argv_data_ = ReadFileString(argv[0], &config->argv_data_size_);
    if (config->argv_data_ == NULL) return 0;
    config->own_argv_ = 1;
    config->argv_ = (const char**)malloc(MAX_ARGC * sizeof(*config->argv_));
    if (config->argv_ == NULL) return 0;

    argc = 0;
    for (cur = strtok(config->argv_data_, sep);
         cur != NULL;
         cur = strtok(NULL, sep)) {
      if (argc == MAX_ARGC) {
