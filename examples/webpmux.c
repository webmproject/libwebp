...
static void PrintHelp(void) {
  ...
  printf("\nNote: if a single file name is passed as argument, the arguments "
         "will be tokenized\n");
  printf("from this file. The file name must not start with "
         "the character '-'.\n");
}
...
static int SetArguments(int argc, const char* argv[],
  ...
  if (argc == 1 && argv[0][0] != '-') {
    char* cur;
    const char sep[] = " \t\r\n\f\v";
    if (!ReadFileToWebPData(argv[0], &config->argv_data_)) return 0;
    config->own_argv_ = 1;
    config->argv_ = (const char**)malloc(MAX_ARGC * sizeof(*config->argv_));
    if (config->argv_ == NULL) return 0;

    argc = 0;
    cur = (char*)config->argv_data_.bytes;
    for (cur = strtok(cur, sep); cur != NULL; cur = strtok(NULL, sep)) {
      if (argc == MAX_ARGC) {
...
static int InitializeConfig(int argc, const char* argv[],
  ...
  if (!SetArguments(argc, argv, config)) return 0;
