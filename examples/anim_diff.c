...
int FramesAreEqual(const uint8_t* const rgba1, const uint8_t* const rgba2,
...
int FramesAreSimilar(const uint8_t* const rgba1, const uint8_t* const rgba2,
  ...
  for (j = 0; j < height; ++j) {
    for (i = 0; i < width; ++i) {
      ...
      if (!PixelsAreSimilar(rgba1[offset], rgba1[offset], max_allowed_diff)) {
        return 0;
      }
    }
  }
