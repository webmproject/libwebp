...
int WebPAnimEncoderRefineRect(
  ...
  rect.x_offset_ = clip(*x_offset, 0, curr_canvas->width - 1);
  rect.y_offset_ = clip(*y_offset, 0, curr_canvas->height - 1);
  rect.width_ = clip(*width, 0, curr_canvas->width - rect.x_offset_);
  rect.height_ = clip(*height, 0, curr_canvas->height - rect.y_offset_);
  MinimizeChangeRectangle(prev_canvas, curr_canvas, &rect,
                          is_lossless, quality);
  SnapToEvenOffsets(&rect);
