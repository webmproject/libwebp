...
static int ReadHuffmanCodes(VP8LDecoder* const dec, int xsize, int ysize,
  ...
  if (allow_recursion && VP8LReadBits(br, 1)) {
    ...
    if (num_htree_groups > 1000 || num_htree_groups > xsize * ysize) {
      num_htree_groups_tmp = (xsize * ysize > 1000) ? 1000 : xsize * ysize;
      memcpy(&br_tmp, &dec->br_, sizeof(dec->br_));
    } else {
      num_htree_groups_tmp = num_htree_groups;
    }
  }
  ...
  while (i != num_htree_groups) {
    huffman_tables = (HuffmanCode*)WebPSafeMalloc(
        num_htree_groups_tmp * table_size, sizeof(*huffman_tables));
    htree_groups = VP8LHtreeGroupsNew(num_htree_groups_tmp);
    ...
    next = huffman_tables;
    for (i = 0; i < num_htree_groups_tmp; ++i) {
    ...
    if (i == num_htree_groups_tmp && num_htree_groups != num_htree_groups_tmp) {
      num_htree_groups_tmp = num_htree_groups;
      WebPSafeFree(huffman_tables);
      VP8LHtreeGroupsFree(htree_groups);
      huffman_tables = NULL;
      htree_groups = NULL;
      memcpy(&dec->br_, &br_tmp, sizeof(dec->br_));
    }
  }
