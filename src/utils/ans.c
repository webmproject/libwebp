...
uint32_t ANSDecReadBit(ANSDec* const ans, const uint32_t p0) {
...
uint32_t ANSDecReadSymbol(ANSDec* const ans,
                          const ANSSymbolInfo codes[ANS_TAB_SIZE]) {
  const uint32_t res = ans->state_ & ANS_TAB_MASK;
  const ANSSymbolInfo s = codes[res];
  ans->state_ = s.freq_ * (ans->state_ >> ANS_LOG_TAB_SIZE) + s.offset_;
  if (ans->state_ < IO_LIMIT_LO) ReadNextWord(ans);
  return s.symbol_;
}
...
void ANSEncDictionaryClear(ANSEncDictionary* const dict) {
  if (dict != NULL) {
    ...
    dict->nb_symbols_ = 0;
    dict->total_ = 0;
    // don't reset max_freq_, final_total_, id_.
  }
}
...
static void EmitBit(ANSEnc* const ans, uint32_t bit, uint32_t p0) {
  ...
  if (p0 == 0) {
    ...
    ans->state_ = s;
    assert(ans->state_ >= IO_LIMIT_LO && (uint64_t)ans->state_ < IO_LIMIT_HI);
  }
}

static void EmitSymbol(ANSEnc* const ans, const ANSSymbolInfo t) {
  uint32_t s = ans->state_;
  if ((s >> (IO_LIMIT_BITS + IO_BITS - ANS_LOG_TAB_SIZE)) >= t.freq_) {
    EmitWord(ans);
    s = ans->state_;
  }
  ans->state_ = ((s / t.freq_) << ANS_LOG_TAB_SIZE) + (s % t.freq_) + t.offset_;
  assert(ans->state_ >= IO_LIMIT_LO && (uint64_t)ans->state_ < IO_LIMIT_HI);
}
...
static int EmitOnlyBits(ANSEnc* const ans) {
  ...
  while (page != NULL) {
    ...
    while (--n >= 0) {
      ...
      const uint32_t p0 = tok & 0x1ffff;
      const int bit = !!(tok & (1 << 29));
  ...
  while (page != NULL) {
    ...
    while (--n >= 0) {
      ...
      if ((tok & ANS_TOK_TYPE) == ANS_TOK_IS_BIT) {
        ...
        EmitUValue(ans, (tok >> 8) & ANS_TOK_UVAL_MASK, tok & 0xff);
      }
    }
    page = page->prev_;
  }
...
static int EmitAll(ANSEnc* const ans,
                   ANSSymbolInfo infos[][ANS_MAX_SYMBOLS]) {
  const ANSTokenPage* page = ans->tokens_;
  while (page != NULL) {
    int n = page->nb_tokens_;
    while (--n >= 0) {
      ...
      if ((tok & ANS_TOK_TYPE) == ANS_TOK_IS_BIT) {
        ...
        EmitSymbol(ans, infos[id][tok & 0xffff]);
      }
    }
    page = page->prev_;
  }
...
// the total is rescaled to be equal to 'final_tab_size' exactly.
// Returns 0 in case of failure. Otherwise, returns 'final_tab_size'.
...
int ANSCountsToSpreadTable(uint32_t counts[], int max_symbol,
                           int target_total_count, ANSSymbolInfo codes[]) {
  int s, pos = 0;
  if (target_total_count > 0 &&
      !ANSNormalizeCounts(counts, max_symbol, target_total_count)) {
    return 0;
  }
  for (s = 0; s < max_symbol; ++s) {
    const uint32_t freq = counts[s];
    uint32_t k;
    for (k = 0; k < freq; ++k, ++pos) {
  ...
  if (ans->nb_symbols_ > 0) {
    ...
    for (d = 0; d < ans->nb_dict_; ++d) {
      ...
      if (dict != NULL && !dict->stats_ok_) {
        ...
        if (dict->final_total_ == 0) {         // if tab size is unspecified...
          dict->final_total_ = ANS_TAB_SIZE;   // ... use default.
        }
...
int ANSDictionaryToCodingTable(const ANSEncDictionary* const dict,
                               ANSSymbolInfo infos[]) {
  if (dict->stats_ok_) {
    if (dict->total_ != dict->final_total_) {
      uint32_t counts[ANS_MAX_SYMBOLS];
      uint32_t total;
      ...
      total = ANSNormalizeCounts(counts, dict->max_symbol_,
                                 dict->final_total_);
      if (total == 0) return 0;
      CountsToInfos(counts, dict->max_symbol_, infos);
    } else {
      CountsToInfos(dict->counts_, dict->max_symbol_, infos);
    }
    return 1;
  }
  return 0;
...
int ANSEncAssemble(ANSEnc* const ans) {
  ...
  if (ans->nb_symbols_ == 0) {
    ...
    ANSSymbolInfo infos[ANS_MAX_DICTIONARIES][ANS_MAX_SYMBOLS];
    uint32_t d;
    for (d = 0; d < ans->nb_dict_; ++d) {
      if (!ANSDictionaryToCodingTable(&ans->dict_[d], infos[d])) {
        return 0;
      }
    }
    return EmitAll(ans, infos);
  }
}
