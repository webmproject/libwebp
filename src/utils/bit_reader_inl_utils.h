...
static WEBP_INLINE int VP8GetBit(VP8BitReader* const br, int prob BT_LABEL) {
...
int VP8GetSigned(VP8BitReader* const br, int v BT_LABEL) {
  if (br->bits_ < 0) {
    VP8LoadNewBytes(br);
  }
  {
...
static WEBP_INLINE int VP8GetBitAlt(VP8BitReader* const br, int prob BT_LABEL) {
  // Don't move this declaration! It makes a big speed difference to store
  // 'range' *before* calling VP8LoadNewBytes(), even if this function doesn't
  // alter br->range_ value.
  range_t range = br->range_;
