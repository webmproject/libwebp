...
/*
  Sample usage:
    ...
    for (n = 0; n < num_dicts; ++n) {
      ...
      dicts[n].final_total_ = 1 << 12;   // final coding table size
    }
    ...
    ANSSymbolInfo codes[num_dictionaries][ANS_TAB_SIZE];
    ...
    for (...) {
      ...
      symbol = ANSDecReadSymbol(&dec, codes[dict]);
      ...
...
// Generate a spread table from a set of frequencies. If target_total_count is
// non zero, the counts[] distributions is re-normalized such that its sum is
// equal to target_total_count.
...
#define ANS_TOK_UVAL_MASK ((1u << 17) - 1)
...
typedef struct {
  ...
  uint32_t final_total_;  // final expect total of counts[], used for coding
                          // MUST be 0 for now! (=default to ANS_TAB_SIZE)
} ANSEncDictionary;
...
// Process a closed dictionary to retrieve infos[] information suitable for
// coding.
