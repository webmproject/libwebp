...
#define MULTIPLY_BY_ALPHA(V, ALPHA, OTHER) do {                        \
  ...
  const uint16x8_t r1 = vmull_u8(rgba.val[1], alpha);                  \
  const uint16x8_t g1 = vmull_u8(rgba.val[2], alpha);                  \
  const uint16x8_t b1 = vmull_u8(rgba.val[(OTHER)], alpha);            \
  ...
  rgba.val[1] = vshrn_n_u16(r3, 8);                                    \
  rgba.val[2] = vshrn_n_u16(g3, 8);                                    \
  rgba.val[(OTHER)] = vshrn_n_u16(b3, 8);                              \
} while (0)
...
static void ApplyAlphaMultiply_NEON(uint8_t* rgba, int alpha_first,
  ...
  while (h-- > 0) {
    ...
    if (alpha_first) {
      for (; i + 8 <= w; i += 8) {
        // load aaaa...|rrr...|gggg...|bb...
        uint8x8x4_t rgba = vld4_u8((const uint8_t*)(rgbx + i));
        MULTIPLY_BY_ALPHA(rgba, 0, 3);
        vst4_u8((uint8_t*)(rgbx + i), rgba);
      }
    } else {
      for (; i + 8 <= w; i += 8) {
        uint8x8x4_t rgba = vld4_u8((const uint8_t*)(rgbx + i));
        MULTIPLY_BY_ALPHA(rgba, 3, 0);
        vst4_u8((uint8_t*)(rgbx + i), rgba);
      }
    }
