...
static void TM4(uint8_t* dst) {
  ...
  const v16i8 zero = { 0 };
  const v8i16 TL = (v8i16)__msa_fill_h(ptemp[0]);
  const v8i16 L0 = (v8i16)__msa_fill_h(ptemp[BPS]);
...
static void VE4(uint8_t* dst) {    // vertical
  ...
  SW4(val0, val0, val0, val0, dst, BPS);
}
...
static void RD4(uint8_t* dst) {   // Down-right
  ...
  A = SLDI_UB(A1, A1, 12);
  A = (v16u8)__msa_insert_b((v16i8)A, 3, ptop[BPS]);
