...
static uint32_t UInt32From16b(const __m128i* const m) {
  uint16_t tmp[4];
  const __m128i a = _mm_srli_si128(*m, 8);
  const __m128i b = _mm_add_epi16(*m, a);
  _mm_storel_epi64((__m128i*)tmp, b);
  return (uint32_t)tmp[0] + tmp[1] + tmp[2] + tmp[3];
}

static uint32_t UInt32From32b(const __m128i* const m) {
  uint32_t tmp[2];
  const __m128i a = _mm_srli_si128(*m, 8);
  const __m128i b = _mm_add_epi32(*m, a);
  _mm_storel_epi64((__m128i*)tmp, b);
  return (uint32_t)tmp[0] + tmp[1];
}
...
static double SSIMGet_SSE2(const uint8_t* src1, int stride1,
  ...
  stats.xm  = UInt32From16b(&xm);
  stats.ym  = UInt32From16b(&ym);
  stats.xxm = UInt32From32b(&xxm);
  stats.xym = UInt32From32b(&xym);
  stats.yym = UInt32From32b(&yym);
