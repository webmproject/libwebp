...
static WEBP_INLINE uint32_t ClampedAddSubtractFull(uint32_t c0, uint32_t c1,
  ...
  const uint16x8 A0 = vmovl_u8(C0);
  const uint16x8 A1 = vmovl_u8(C1);
  const uint16x8 sum = vadd_u16(A0, A1);
  const uint16x8 A2 = vmovl_u8(C2);
  // Saturated subtract.
  const uint16x8 diff = vqsub_u16(sum, A2);
  // Saturated narrow conversion.
  const uint8x8 res = vqmovn_u16(diff);
