...
#define DO_PRED12(DIFF, LANE, OUT) do {                                \
  const uint8x8_t pred = vqmovun_s16(vaddq_s16(left, (DIFF)));         \
  const uint8x8_t res = vadd_u8(pred, (OUT <= 1) ? vget_low_u8(src)    \
                                                 : vget_high_u8(src)); \
  vst1_lane_u32(&out[i + (OUT)], vreinterpret_u8_u32(res), (LANE));    \
  /* rotate in the left predictor for next iteration */                \
  left = (LANE == 0) ? vextq_s16(zero, vmovl_u8(res), 4)               \
                     : vextq_s16(vmovl_u8(res), zero, 4);              \
} while (0)

static void PredictorAdd12_NEON(const uint32_t* in, const uint32_t* upper,
                                int num_pixels, uint32_t* out) {
  int i;
  const int8x16_t zero = vdupq_n_s32(0);
  int16x8_t left = vreinterpretq_s16_u16(vmovl_u8(LOAD_U32_AS_U8(out[0 - 1])));
  for (i = 0; i + 4 <= num_pixels; i += 4) {
