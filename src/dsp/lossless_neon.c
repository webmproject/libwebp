...
static void PredictorAdd11_SSE(const uint32_t* in, const uint32_t* upper,
  ...
  const uint8x16_t zeroq_u8 = LOAD_U32_AS_U8Q(0);
  const uint32x4_t zeroq_u32 = vdupq_n_u32(0);
  for (i = 0; i < num_pixels - 4; i += 4) {
    const uint8x16_t T = LOAD_U32P_AS_U8Q(&upper[i]);
    ...
    for (j = 0; j < 4; ++j) {
      ...
      const uint32x2_t srcL =
          vreinterpret_u32_u8(vadd_u8(vget_high_u8(src), L));
      // Add to top (pre-computed) or left.
      const uint32x2_t output = vbsl_u32(cmp, vget_high_u32(sumTin), srcL);
      out[i + j] = vget_lane_u32(output, 0);
      // Shift the pre-computed value for the next iteration.
      pTTL = vextq_u8(zeroq_u8, pTTL, 12);
      TL = vextq_u8(zeroq_u8, TL, 12);
      src = vextq_u8(zeroq_u8, src, 12);
      sumTin = vextq_u32(zeroq_u32, sumTin, 3);
    }
  }
