...
static WEBP_INLINE uint32_t ClampedAddSubtractHalf(uint32_t c0, uint32_t c1,
                                                   uint32_t c2) {
  const int16x8_t C0 =
      vreinterpret_s16_u16(vmovl_u8(vreinterpretq_u8_u32(vld1_dup_u32(&c0))));
  const int16x8_t C1 =
      vreinterpret_s16_u16(vmovl_u8(vreinterpretq_u8_u32(vld1_dup_u32(&c1))));
  const int16x8_t C2 =
      vreinterpret_s16_u16(vmovl_u8(vreinterpretq_u8_u32(vld1_dup_u32(&c2))));
  // Average.
  const int16x8_t avg = vhadd_u8(C0, C1);
  const int16x8_t diff_avg = vhsub_s16(avg, C2);
  // Compute the sum and saturate.
  const uint8x8_t res = vqmovun_s16(vadd_s16(avg, diff_avg));
  const uint32_t output = vget_lane_u32(vreinterpret_u32_u8(res), 0);
  return output;
}
