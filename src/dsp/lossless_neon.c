...
static WEBP_INLINE uint32_t Average2(uint32_t a0, uint32_t a1) {
  const uint32x2_t A = vld1_dup_u32(&a0);
  const uint32x2_t B = vld1_dup_u32(&a1);
  const uint32x2_t avg_32x2 = vhadd_u32(A, B);
...
static WEBP_INLINE uint32_t Average3(uint32_t a0, uint32_t a1, uint32_t a2) {
  const uint32x2_t A = vld1_dup_u32(&a0);
  const uint32x2_t C = vld1_dup_u32(&a2);
  const uint32x2_t avg1 = vhadd_u32(A, C);
  const uint32x2_t B = vld1_dup_u32(&a1);
  const uint32x2_t avg2 = vhadd_u32(avg1, B);
...
static WEBP_INLINE uint32_t Average4(uint32_t a0, uint32_t a1,
  ...
  const uint32x2_t A = vld1_dup_u32(&a0);
  const uint32x2_t B = vld1_dup_u32(&a1);
  const uint32x2_t avg1 = vhadd_u32(A, B);
  const uint32x2_t C = vld1_dup_u32(&a2);
  const uint32x2_t D = vld1_dup_u32(&a3);
  const uint32x2_t avg2 = vhadd_u32(C, D);
  const uint32x2_t avg3 = vhadd_u32(avg1, avg2);
  const uint32_t avg = vget_lane_u32(avg3, 0);
  return avg;
}
