...
static WEBP_INLINE uint32_t ClampedAddSubtractHalf_NEON(uint32_t c0,
                                                        uint32_t c1,
                                                        uint32_t c2) {
  const uint8x8_t avg_u8 = Average2_u8_NEON(c0, c1);
  // Average.
  const int16x8_t avg = vreinterpretq_s16_u16(vmovl_u8(avg_u8));
  const uint8x8_t C2_u8 = vreinterpret_u8_u32(vdup_n_u32(c2));

  const uint8x8_t cmp = vcgt_u8(C2_u8, avg_u8);
  C2_u8 = vadd_u8(C2_u8, cmp);
  const int8x8_t diff_avg = vreinterpretq_s8_u8(vhsub_u8(avg_u8, C2_u8));
  const uint8x8_t res = vqmovun_s16(vaddw_s8(avg, diff_avg));
#if 0
  const int16x8_t C2 = vreinterpretq_s16_u16(vmovl_u8(C2_u8));
  const int16x8_t diff = vsubq_s16(avg, C2);
  const int16x8_t cmp = vreinterpretq_s16_u16(vcgtq_s16(C2, avg));
  const int16x8_t diff_avg = vhsubq_s16(diff, cmp);
...
static WEBP_INLINE uint32_t Select_NEON(uint32_t a, uint32_t b, uint32_t c) {
  const uint8x8_t A = vreinterpret_u8_u32(vdup_n_u32(a));
  const uint8x8_t C = vreinterpret_u8_u32(vdup_n_u32(c));
  const uint8x8_t pa = vabd_u8(A, C);
  const uint8x8_t B = vreinterpret_u8_u32(vdup_n_u32(b));
