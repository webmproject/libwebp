...
#define LOADQ_U32_AS_U8(VALUE) vreinterpretq_u8_u32(vdupq_n_u32((VALUE)))
#define LOADQ_U32P_AS_U8(VALUE) vreinterpretq_u8_u32(vld1q_u32((VALUE)))
...
static void PredictorAdd11_SSE(const uint32_t* in, const uint32_t* upper,
  ...
  for (i = 0; i + 4 <= num_pixels; i += 4) {
    ...
    for (j = 0; j < 4; ++j) {
      ...
      const uint8x8_t pLTL = vabd_u8(L, vget_high_u8(TL));  // |L - TL|
      const int16x4_t diff = vget_high_s16(
          vreinterpretq_s16_u16(vsubl_u8(pLTL, vget_high_u8(pTTL))));
      ...
      const uint32x2_t sumLin =
          vreinterpret_u32_u8(vadd_u8(L, vget_high_u8(src)));
      // Add to top (pre-computed) or left.
      const uint32x2_t output = vbsl_u32(cmp, vget_high_u32(sumTin), sumLin);
      out[i + j] = vget_lane_u32(output, 0);
      // Rotate the pre-computed value for the next iteration.
      pTTL = vextq_u8(pTTL, pTTL, 12);
      TL = vextq_u8(TL, TL, 12);
      src = vextq_u8(src, src, 12);
      sumTin = vextq_u32(sumTin, sumTin, 3);
    }
  }
