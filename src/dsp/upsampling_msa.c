...
static void YuvToRgba4444Line(const uint8_t* y, const uint8_t* u,
  ...
  if (length > 8) {
    uint8_t temp[32];
  #ifdef WEBP_SWAP_16BIT_CSP
    CALC_RGBA4444(y, u, v, BA, RG, 16, &temp[0]);
  #else
    CALC_RGBA4444(y, u, v, RG, BA, 16, &temp[0]);
  #endif
    memcpy(dst, &temp[0], length * 2 * sizeof(*dst));
  } else if (length > 0) {
    uint8_t temp[32];
  #ifdef WEBP_SWAP_16BIT_CSP
    CALC_RGBA4444(y, u, v, BA, RG, 8, &temp[0]);
  #else
    CALC_RGBA4444(y, u, v, RG, BA, 8, &temp[0]);
  #endif
    memcpy(dst, &temp[0], length * 2 * sizeof(*dst));
  }
}
...
static void YuvToRgb565Line(const uint8_t* y, const uint8_t* u,
  ...
  if (length > 8) {
    uint8_t temp[32];
  #ifdef WEBP_SWAP_16BIT_CSP
    CALC_RGB565(y, u, v, GB, RG, 16, &temp[0]);
  #else
    CALC_RGB565(y, u, v, RG, GB, 16, &temp[0]);
  #endif
    memcpy(dst, &temp[0], length * 2 * sizeof(*dst));
  } else if (length > 0) {
    uint8_t temp[32];
  #ifdef WEBP_SWAP_16BIT_CSP
    CALC_RGB565(y, u, v, GB, RG, 8, &temp[0]);
  #else
    CALC_RGB565(y, u, v, RG, GB, 8, &temp[0]);
  #endif
    memcpy(dst, &temp[0], length * 2 * sizeof(*dst));
  }
}
