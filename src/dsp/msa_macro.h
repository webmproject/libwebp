...
#define MSA_LOAD(psrc, TYPE, INSTR) ( {         \
...
#define MSA_STORE(val, pdst, TYPE, INSTR) {      \
...
#if (__mips_isa_rev >= 6)
  #define LH(psrc)  MSA_LOAD(psrc, uint16_t, lh)
  #define LW(psrc)  MSA_LOAD(psrc, uint32_t, lw)
  #if (__mips == 64)
    #define LD(psrc)  MSA_LOAD(psrc, uint64_t, ld)
  #else  // !(__mips == 64)
    #define LD(psrc) ( {                                       \
      ...
      out_m = (uint64_t)(val1_m);                              \
      out_m = (uint64_t)((out_m << 32) & 0xFFFFFFFF00000000);  \
      out_m = (uint64_t)(out_m | (uint64_t) val0_m);           \
      out_m;                                                   \
    } )
  #endif  // (__mips == 64)

  #define SH(val, pdst)  MSA_STORE(val, pdst, uint16_t, sh)
  #define SW(val, pdst)  MSA_STORE(val, pdst, uint32_t, sw)
  #define SD(val, pdst)  MSA_STORE(val, pdst, uint64_t, sd)
#else  // !(__mips_isa_rev >= 6)
  #define LH(psrc)  MSA_LOAD(psrc, uint16_t, ulh)
  #define LW(psrc)  MSA_LOAD(psrc, uint32_t, ulw)
  #if (__mips == 64)
    #define LD(psrc)  MSA_LOAD(psrc, uint64_t, uld)
  #else  // !(__mips == 64)
    #define LD(psrc) ( {                                       \
      uint8_t *psrc_ld_m = (uint8_t*)(psrc);                   \
      uint32_t val0_m, val1_m;                                 \
  ...
  #define SH(val, pdst)  MSA_STORE(val, pdst, uint16_t, ush)
  #define SW(val, pdst)  MSA_STORE(val, pdst, uint32_t, usw)
  #define SD(val, pdst) {                                     \
