...
#define MULTIPLY_16b(r, g, b, C0, C1, C2, CST, DST_s16) do {     \
...
static uint8x8_t ConvertRGBToU_NEON(const uint16x8_t r,
                                    const uint16x8_t g,
                                    const uint16x8_t b) {
  int16x8_t U;
  MULTIPLY_16b(r, g, b, -9719, -19081, 28800, 128 << 2, U);
  return vqrshrun_n_s16(U, 2);
}

static uint8x8_t ConvertRGBToV_NEON(const uint16x8_t r,
                                    const uint16x8_t g,
                                    const uint16x8_t b) {
  int16x8_t V;
  MULTIPLY_16b(r, g, b, 28800, -24116, -4684, 128 << 2, V);
  return vqrshrun_n_s16(V, 2);
}
...
static void ConvertRGBA32ToUV_NEON(const uint16_t* rgb,
  ...
  for (i = 0; i + 8 <= width; i += 8, rgb += 4 * 8) {
    ...
    vst1_u8(u + i, ConvertRGBToU_NEON(RGB.val[0], RGB.val[1], RGB.val[2]));
    vst1_u8(v + i, ConvertRGBToV_NEON(RGB.val[0], RGB.val[1], RGB.val[2]));
  }
