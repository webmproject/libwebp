...
static void FUNC_NAME(const uint8_t* top_y, const uint8_t* bottom_y,           \
  ...
  if (len > 1) {                                                               \
    ...
    uint8_t* const tmp_top = r_u + 4 * 32;                                     \
    uint8_t* const tmp_bottom = (bottom_y == NULL) ? NULL : tmp_top + 4 * 32;  \
    assert(left_over > 0);                                                     \
    UPSAMPLE_LAST_BLOCK(top_u + uv_pos, cur_u + uv_pos, left_over, r_u);       \
    UPSAMPLE_LAST_BLOCK(top_v + uv_pos, cur_v + uv_pos, left_over, r_v);       \
    memcpy(tmp_top, top_y + pos, len - pos);                                   \
    if (bottom_y != NULL) memcpy(tmp_bottom, bottom_y + pos, len - pos);       \
    CONVERT2RGB_32(FUNC, XSTEP, tmp_top, tmp_bottom, tmp_top, tmp_bottom, 0);  \
    memcpy(top_dst + pos * (XSTEP), tmp_top, (len - pos) * (XSTEP));           \
    if (bottom_y != NULL) {                                                    \
      memcpy(bottom_dst + pos * (XSTEP), tmp_bottom, (len - pos) * (XSTEP));   \
    }                                                                          \
  }                                                                            \
}
