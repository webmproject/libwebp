...
static void AddGreenToBlueAndRed(const uint32_t* const src, int num_pixels,
  ...
  "2:                                                         \n\t"
    ".set       pop                                           \n\t"
    : [dst]"=r"(dst), [temp0]"=&r"(temp0),
      [temp1]"=&r"(temp1), [temp2]"=&r"(temp2), [temp3]"=&r"(temp3),
      [temp4]"=&r"(temp4), [temp5]"=&r"(temp5), [temp6]"=&r"(temp6),
      [temp7]"=&r"(temp7)
    : [p_loop1_end]"r"(p_loop1_end), [src]"r"(src),
      [p_loop2_end]"r"(p_loop2_end)
    : "memory"
  );
}
...
static void TransformColorInverse(const VP8LMultipliers* const m,
  ...
  "1:                                                        \n\t"
    ...
    : [temp0]"=&r"(temp0), [temp1]"=&r"(temp1), [temp2]"=&r"(temp2),
      ...
      [argb1]"=&r"(argb1), [dst]"=r"(dst)
    : [G_to_R]"r"(G_to_R), [R_to_B]"r"(R_to_B),
      [G_to_B]"r"(G_to_B), [src]"r"(src), [p_loop_end]"r"(p_loop_end)
    : "memory", "hi", "lo"
  );
