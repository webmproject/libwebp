...
static void AddGreenToBlueAndRed(const uint32_t* const src, int num_pixels,
  ...
  uint32_t* const p_loop1_end = src + (num_pixels & ~3);
  uint32_t* const p_loop2_end = src + num_pixels;
  __asm__ volatile (
    ".set       push                                          \n\t"
    ".set       noreorder                                     \n\t"
  ...
  "2:                                                         \n\t"
    ".set       pop                                           \n\t"
    : [src]"r"(src), [dst]"=r"(dst), [temp0]"=&r"(temp0),
      [temp1]"=&r"(temp1), [temp2]"=&r"(temp2), [temp3]"=&r"(temp3),
...
static void TransformColorInverse(const VP8LMultipliers* const m,
  ...
  uint32_t* const p_loop_end = src + (num_pixels & ~1);
  __asm__ volatile (
    ".set            push                                    \n\t"
    ".set            noreorder                               \n\t"
  ...
  "1:                                                        \n\t"
    ...
    : [temp0]"=&r"(temp0), [temp1]"=&r"(temp1), [temp2]"=&r"(temp2),
      ...
      [argb1]"=&r"(argb1), [src]"r"(src), [dst]"=r"(dst)
