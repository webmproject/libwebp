...
static WEBP_INLINE void AddPixels_SSE(const __m128i* const a,
  ...
  *res = _mm_packus_epi16(sum0_mask, sum1_mask);
}
...
static void PredictorAdd11_SSE(const uint32_t* in, const uint32_t* upper,
  ...
  for (i = 0; i + 4 <= num_pixels; i += 4) {
    ...
    const __m128i top = _mm_loadu_si128((const __m128i*)&upper[i]);
    __m128i sumTin;
    AddPixels_SSE(&src, &top, &sumTin);
    for (j = 0; j < 4; ++j) {
      int pa_minus_pb;
      ...
      if (pa_minus_pb <= 0) {
        ...
        // Add to left.
        __m128i tmp;
        AddPixels_SSE(&src, &L, &tmp);
        out[i + j] = _mm_cvtsi128_si32(tmp);
      }
