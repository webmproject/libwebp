...
#define GRAD_PROCESS_LANE(L)  do {  \
  const int16x8_t tmp3 = vaddq_s16(BC, vreinterpretq_s16_u16(vmovl_u8(out)));  \
  const uint8x8_t delta = vqmovun_s16(tmp3);                                   \
  const uint8x8_t final = vadd_u8(D, delta);                                   \
  out = vext_u8(vrev64_u8(final), out, 7);   /* rotate new result in */        \
  if ((L) < 7) {       /* rotate the predictors right */                       \
    BC = vextq_s16(BC, BC, 1);                                                 \
    D = vext_u8(D, D, 1);                                                      \
  }                                                                            \
} while (0)
...
static void GradientPredictInverse_NEON(const uint8_t* const in,
  ...
  if (length > 0) {
    int i;
    uint8x8_t out = vdup_n_u8(row[-1]);   // left sample
    for (i = 0; i + 8 <= length; i += 8) {
      const int16x8_t B = LOAD_U8_TO_S16(&top[i + 0]);
      const int16x8_t C = LOAD_U8_TO_S16(&top[i - 1]);
      int16x8_t BC = vsubq_s16(B, C);  // unclipped gradient basis B - C
      uint8x8_t D = vld1_u8(&in[i]);   // base input
      GRAD_PROCESS_LANE(0);
      GRAD_PROCESS_LANE(1);
      GRAD_PROCESS_LANE(2);
      GRAD_PROCESS_LANE(3);
