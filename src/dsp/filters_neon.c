...
static void HorizontalUnfilter_NEON(const uint8_t* prev, const uint8_t* in,
  ...
  int i;
  const uint16x8_t zero = vdupq_n_u8(0);
  uint16x8_t last;
  out[0] = in[0] + (prev == NULL ? 0 : prev[0]);
  if (width <= 1) return;
  last = vsetq_lane_u8(out[0], zero, 0);
  for (i = 1; i + 16 <= width; i += 16) {
    const uint16x8_t A0 = vld1q_u8(&in[i]);
    const uint16x8_t A1 = vaddq_u8(A0, last);
    const uint16x8_t A2 = SHIFT_LEFT_N_Q(A1, 1);
    const uint16x8_t A3 = vaddq_u8(A1, A2);
    const uint16x8_t A4 = SHIFT_LEFT_N_Q(A3, 2);
    const uint16x8_t A5 = vaddq_u8(A3, A4);
    const uint16x8_t A6 = SHIFT_LEFT_N_Q(A5, 4);
    const uint16x8_t A7 = vaddq_u8(A5, A6);
    const uint16x8_t A8 = SHIFT_LEFT_N_Q(A7, 8);
    const uint16x8_t A9 = vaddq_u8(A7, A8);
    vst1q_u8(&out[i], A9);
