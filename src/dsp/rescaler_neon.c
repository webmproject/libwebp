...
#define MULT_FIX_FLOOR(A, B) /* note: B is actualy scale>>1. See MAKE_HALF_CST */ \
...
static void RescalerExportRowShrink_NEON(WebPRescaler* const wrk) {
  ...
  if (yscale) {
    ...
    for (x_out = 0; x_out < max_span; x_out += 8) {
      ...
      const uint32x4_t A0 = MULT_FIX_FLOOR(in0, fxy_scale_half);
      const uint32x4_t A1 = MULT_FIX_FLOOR(in1, fxy_scale_half);
    ...
    for (; x_out < x_out_max; ++x_out) {
      const int v = (int)MULT_FIX_FLOOR_C(irow[x_out], fxy_scale);
      assert(v >= 0 && v <= 255);
      dst[x_out] = v;
      irow[x_out] = 0;
    }
