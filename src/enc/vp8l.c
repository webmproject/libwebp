...
static WEBP_INLINE uint32_t ApplyPaletteHash2(uint32_t color) {
  // Forget about alpha.
  return (uint32_t)(((color & 0x00ffffffu) * 4222244071u)) >>
         (32 - PALETTE_INV_SIZE_BITS);
}

static WEBP_INLINE uint32_t ApplyPaletteHash3(uint32_t color) {
  // Forget about alpha.
  return (uint32_t)((color & 0x00ffffffu) * ((1u << 31) - 1)) >>
         (32 - PALETTE_INV_SIZE_BITS);
}
  ...
  if (palette_size < APPLY_PALETTE_GREEDY_MAX) {
    ...
    if (use_LUT) {
      if (i == 1) {
        ...
        APPLY_PALETTE_FOR(buffer[ApplyPaletteHash3(pix)])
      }
    } else {
