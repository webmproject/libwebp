...
static void InterpolateTwoRows(const fixed_y_t* const best_y,
                               ...
                               fixed_y_t* out2) {
  const int uv_w = w >> 1;
  const int flt_len = (w - 1) >> 1;   // length to filter
  int k = 3;
  while (k-- > 0) {   // process each R/G/B segments in turn
    ...
    WebPSmartYUVFilterRow(cur_uv, prev_uv, flt_len, best_y + 0 + 1, out1 + 1);
    WebPSmartYUVFilterRow(cur_uv, next_uv, flt_len, best_y + w + 1, out2 + 1);
