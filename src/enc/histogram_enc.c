...
static int HistogramCombineStochastic(VP8LHistogramSet* const image_histo,
  ...
  for (iter = 0;
    ...
    int j;
    const uint32_t rand_range = image_histo->size;
    // image_histo->size / 2 was chosen empirically. Less means faster but worse
    // compression.
    const int num_tries = image_histo->size / 2;

    // Pick random samples.
    for (j = 0; rand_range >= 2 && j < num_tries; ++j) {
      double curr_cost;
      uint32_t idx1, idx2;
      // Choose two different histograms at random and try to combine them.
      idx1 = MyRand(&seed) % (rand_range - 1);
      while (histograms[idx1] == NULL) idx1 = (idx1 + 1) % (rand_range - 1);
      idx2 = idx1 + 1 + (MyRand(&seed) % (rand_range - (idx1 + 1)));
      while (histograms[idx2] == NULL || idx1 == idx2) {
        idx2 = (idx2 + 1) % rand_range;
      }
...
static void OptimizeHistogramSymbols(const VP8LHistogramSet* const set,
  ...
  for (i = 0; i < set->max_size; ++i) {
    ...
    const int cluster = cluster_mappings[symbols[i]];
