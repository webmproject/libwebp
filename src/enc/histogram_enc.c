...
static void HistogramClear(VP8LHistogram* const p) {
  ...
  memset(p->is_empty_, 1, sizeof(p->is_empty_));
...
static double PopulationCost(const uint32_t* const population, int length,
  ...
  // The histogram is empty if there are no non-zero streaks.
  *is_empty = (stats.streaks[1][0] == 0 && stats.streaks[1][1] == 0);
...
static WEBP_INLINE double GetCombinedEntropy(const uint32_t* const X,
  ...
  if (trivial_at_end) {
    ...
    stats.counts[0] = 1;
    // TODO(vrabaud) repalce 1 with (length - 1) > 3
    ...
    if (is_X_empty) {
      ...
      if (is_Y_empty) {
        //VP8LStreaks stats2;
        //VP8LBitEntropy bit_entropy2;
        VP8LGetEntropyUnrefined(X, length, &bit_entropy, &stats);
        //VP8LGetCombinedEntropyUnrefined(X, Y, length, &bit_entropy2, &stats2);
        //assert(memcmp(&bit_entropy,&bit_entropy2,sizeof(VP8LBitEntropy))==0);
        //assert(memcmp(&stats,&stats2,sizeof(VP8LStreaks))==0);
      } else {
...
double VP8LHistogramEstimateBits(VP8LHistogram* const p) {
  return
      PopulationCost(p->literal_, VP8LHistogramNumCodes(p->palette_code_bits_),
                     NULL, &p->is_empty_[0])
      + PopulationCost(p->red_, NUM_LITERAL_CODES, NULL, &p->is_empty_[1])
      + PopulationCost(p->blue_, NUM_LITERAL_CODES, NULL, &p->is_empty_[2])
      + PopulationCost(p->alpha_, NUM_LITERAL_CODES, NULL, &p->is_empty_[3])
      + PopulationCost(p->distance_, NUM_DISTANCE_CODES, NULL, &p->is_empty_[4])
...
static int GetCombinedHistogramEntropy(const VP8LHistogram* const a,
  ...
  *cost +=
      GetCombinedEntropy(a->red_, b->red_, NUM_LITERAL_CODES, a->is_empty_[1],
                         b->is_empty_[1], trivial_at_end);
  if (*cost > cost_threshold) return 0;

  *cost +=
      GetCombinedEntropy(a->blue_, b->blue_, NUM_LITERAL_CODES, a->is_empty_[2],
                         b->is_empty_[2], trivial_at_end);
  if (*cost > cost_threshold) return 0;

  *cost +=
      GetCombinedEntropy(a->alpha_, b->alpha_, NUM_LITERAL_CODES,
                         a->is_empty_[3], b->is_empty_[3], trivial_at_end);
  if (*cost > cost_threshold) return 0;

  *cost +=
      GetCombinedEntropy(a->distance_, b->distance_, NUM_DISTANCE_CODES,
                         a->is_empty_[4], b->is_empty_[4], 0);
  *cost +=
...
static void UpdateHistogramCost(VP8LHistogram* const h) {
  ...
  const double distance_cost =
      PopulationCost(h->distance_, NUM_DISTANCE_CODES, NULL, &h->is_empty_[4]) +
      VP8LExtraCost(h->distance_, NUM_DISTANCE_CODES);
  const int num_codes = VP8LHistogramNumCodes(h->palette_code_bits_);
  h->literal_cost_ =
      PopulationCost(h->literal_, num_codes, NULL, &h->is_empty_[0]) +
          VP8LExtraCost(h->literal_ + NUM_LITERAL_CODES, NUM_LENGTH_CODES);
  h->red_cost_ =
      PopulationCost(h->red_, NUM_LITERAL_CODES, &red_sym, &h->is_empty_[1]);
  h->blue_cost_ =
      PopulationCost(h->blue_, NUM_LITERAL_CODES, &blue_sym, &h->is_empty_[2]);
...
static void HistogramCopyAndAnalyze(
  ...
  for (i = 0; i < histo_size; ++i) {
    ...
    if (histo->is_empty_[0] && histo->is_empty_[1] && histo->is_empty_[2]
        && histo->is_empty_[3] && histo->is_empty_[4]) {
      continue;
    }
