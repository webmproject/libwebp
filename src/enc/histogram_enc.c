...
// 48271 (and not the original 16807), and a modulo constant of 2^31 − 1.
...
static void HistogramCombineStochastic(VP8LHistogramSet* const image_histo,
  ...
  *do_greedy = (image_histo->size <= min_cluster_size);
  ...
  for (iter = 0; iter < outer_iters && image_histo_size >= min_cluster_size &&
    ...
    // This value was chosen to keep performance on par with previous versions
    // of this algorithm. Less would result in faster but worse compression.
    const int num_tries = (7 * image_histo_size) / 10;
