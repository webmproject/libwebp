...
static double PopulationCost(const uint32_t* const population, int length,
  ...
  // The histogram is empty if the longest streak of 0's is length.
  *is_empty = (stats.counts[0] == length);
...
static void HistogramCopyAndAnalyze(
  ...
  for (i = 0; i < histo_size; ++i) {
    ...
    // Skip the histogram if it is completely empty.
