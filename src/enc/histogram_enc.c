...
static int HistogramCombineStochastic(VP8LHistogramSet* const image_histo,
  ...
  if (!HistoQueueInit(&histo_queue, COST_HEAP_SIZE_SQRT)) {
    goto End;
  }
  ...
  *do_greedy = (image_histo->size <= min_cluster_size);
  ...
  for (iter = 0; iter < outer_iters && image_histo_size >= min_cluster_size &&
    ...
    const int num_tries = (5 * image_histo_size) / 10;
    ...
    *do_greedy |= (tries_with_no_success > 10) &&
    ...
    for (j = 0; j < histo_queue.size;) {
      ...
      int do_eval = 0;
      // The front pair pair could have been duplicated by a random pick so
