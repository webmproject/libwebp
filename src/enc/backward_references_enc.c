...
static VP8LBackwardRefs* GetBackwardReferences(
    int width, int height, const uint32_t* const argb, int quality, int lz77s,
    int* const cache_bits, const VP8LHashChain* const hash_chain,
  ...
  for (i = 1; lz77s; lz77s &= ~i, i <<= 1) {
    int res;
    double bit_cost;
    int cache_bits_tmp = cache_bits_initial;
    if ((lz77s & i) == 0) continue;
    switch (i) {
      case kLZ77RLE:
    ...
    if (i_best == 0 || bit_cost < bit_cost_best) {
  ...
  // costly).
  if (i_best == kLZ77Standard && quality >= 25) {
...
VP8LBackwardRefs* VP8LGetBackwardReferences(
    ...
    int low_effort, int lz77s, int* const cache_bits,
  ...
  if (low_effort) {
    ...
    return GetBackwardReferences(width, height, argb, quality, lz77s,
                                 cache_bits, hash_chain, refs_tmp1, refs_tmp2);
  }
}
