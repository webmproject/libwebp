...
// Compute an LZ77 by forcing matches to happen within a window of a pixel.
#define LZ77_BOX_WINDOWS_SIZE 3
static int BackwardReferencesLz77Box(int xsize, int ysize,
  ...
  const int pix_count = xsize * ysize;
  uint16_t* counts;
  int window_offsets[(LZ77_BOX_WINDOWS_SIZE + 1) *
                     (2 * LZ77_BOX_WINDOWS_SIZE + 1)];
  ...
  // Figure out how many times a pixel is repeated.
  i = pix_count - 2;
  counts = counts_ini + i;
  counts[1] = 1;
  for (; i >= 0; --i, --counts) {
    if (argb[i] == argb[i + 1]) {
      ...
      *counts = *(counts + 1) + (*(counts + 1) != MAX_LENGTH);
    } else {
      *counts = 1;
    }
  }

  // Figure out the window offsets around a pixel.
  {
    int x, y;
    for (y = 0; y <= LZ77_BOX_WINDOWS_SIZE; ++y) {
      for (x = -LZ77_BOX_WINDOWS_SIZE; x <= LZ77_BOX_WINDOWS_SIZE; ++x) {
        const int val = y * xsize - x;
        // Ignore offsets that bring us after the pixel.
        if (val <= 0) continue;
        assert(window_offsets_size <
               sizeof(window_offsets) / sizeof(window_offsets[0]));
  ...
  for (i = pix_count - 1; i > 0; --i) {
    ...
    if (best_length >= MAX_LENGTH) {
      ...
      // window.
      best_distance = VP8LHashChainFindOffset(hash_chain_best, i);
      for (ind = 0; ind < window_offsets_size; ++ind) {
        if (best_distance == window_offsets[ind]) {
          do_compute = 0;
          break;
        }
      }
    ...
    if (do_compute) {
      ...
      for (ind = 0; ind < window_offsets_size; ++ind) {
        ...
        if (best_length < curr_length) {
          best_length = curr_length > MAX_LENGTH ? MAX_LENGTH : curr_length;
          best_distance = window_offsets[ind];
        }
      }
    }
...
static VP8LBackwardRefs* GetBackwardReferences(
  ...
  VP8LHashChain *hash_chain_box = NULL;
  ...
  for (lz77_type = 1; lz77_types_to_try;
    ...
    switch (lz77_type) {
      ...
      case kLZ77Box:
        hash_chain_box =
            (VP8LHashChain*)WebPSafeCalloc(1ULL, sizeof(*hash_chain_box));
        if (hash_chain_box == NULL) goto Error;
...
Error:
  if (hash_chain_box != NULL) {
    VP8LHashChainClear(hash_chain_box);
    WebPSafeFree(hash_chain_box);
  }
