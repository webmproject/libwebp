...
static WEBP_INLINE void PushInterval(CostManager* const manager,
  ...
  // interval logic, just serialize it right away.
  // This constant is empirical.
  const int skip_distance = 10;

  if (len < skip_distance) {
    for (i = index; i < (size_t)(index + len); ++i) {
      const int k = i - index;
      float cost_tmp;
      assert(k >= 0 && k < MAX_LENGTH);
