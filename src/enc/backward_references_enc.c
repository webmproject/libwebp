...
static int BackwardReferencesLz77Box(int xsize, int ysize,
  ...
  for (i = 1; i < pix_count; ++i) {
    ...
    if (do_compute) {
      ...
      // as an initial guess and therefore only introspect the offsets in
      // window_offsets_new.
      int use_prev = (best_length_prev > 1 && best_length_prev < MAX_LENGTH);
      int ind_nbr = use_prev ? window_offsets_new_size : window_offsets_size;
      best_length = use_prev ? best_length_prev - 1 : 0;
      best_offset = use_prev ? best_offset_prev : 0;
      // Find the longest match in a window around the pixel.
      for (ind = 0; ind < ind_nbr; ++ind) {
        int curr_length = 0;
        int j = i;
        int j_offset =
    ...
    if (best_length <= MIN_LENGTH) {
      ...
      best_offset_prev = best_length_prev = 0;
    } else {
