...
static int AnalyzeAndInit(VP8LEncoder* const enc,
                          int entropy_ixs[kNumEntropyIx], int* n_entropy_ixs,
  ...
  if (low_effort) {
    ...
    entropy_ixs[0] = use_palette ? kPalette : kSpatialSubGreen;
    *n_entropy_ixs = 1;
  } else {
    ...
    if (method < 6) {
      ...
      entropy_ixs[0] = min_entropy_ix;
      *n_entropy_ixs = 1;
    } else {
      // Go brute force on all transforms.
      if (config->near_lossless < 100) {
        // kPalette, kSpatial and kSpatialSubGreen are not used in near
        // lossless.
        // kSubGreen.
        entropy_ixs[0] = 0;
        entropy_ixs[1] = kSubGreen;
        *n_entropy_ixs = 2;
      } else {
        int i;
        *n_entropy_ixs = 0;
        for (i = 0; i < kNumEntropyIx; ++i) {
          if (i != kPalette || use_palette) {
            entropy_ixs[(*n_entropy_ixs)++] = i;
            assert(*n_entropy_ixs <= kNumEntropyIx);
          }
        }
      }
    }
...
WebPEncodingError VP8LEncodeStream(const WebPConfig* const config,
  ...
  if (!AnalyzeAndInit(enc, entropy_ixs, &n_entropy_ixs,
                      &red_and_blue_always_zero)) {
    err = VP8_ENC_ERROR_OUT_OF_MEMORY;
    goto Error;
  }

  for (i = 0; i < n_entropy_ixs; ++i) {
    enc->use_palette_ = (entropy_ixs[i] == kPalette);
    enc->use_subtract_green_ = (entropy_ixs[i] == kSubGreen) ||
                               (entropy_ixs[i] == kSpatialSubGreen);
    enc->use_predict_ = (entropy_ixs[i] == kSpatial) ||
                        (entropy_ixs[i] == kSpatialSubGreen);
    if (low_effort) {
    ...
    if (!use_delta_palette) {
      ...
      if (enc->argb_ == NULL || (n_entropy_ixs > 1 && !enc->use_palette_)) {
    ...
    if (best_size == 0 || VP8LBitWriterNumBytes(bw) < best_size) {
      ...
      if (n_entropy_ixs > 1) VP8LBitWriterCopy(bw, &bw_best);
      // Update the stats.
    ...
    if (n_entropy_ixs > 1) VP8LBitWriterReset(&bw_init, bw);
  }
  if (n_entropy_ixs > 1) VP8LBitWriterCopy(&bw_best, bw);
