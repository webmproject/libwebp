...
WebPEncodingError VP8LEncodeStream(const WebPConfig* const config,
    ...
    for (idx = 0; idx < params_size; ++idx) {
      ...
      if (idx == 0) {
        ...
        if (picture->stats == NULL) {
          ...
          param->stats_ = &stats_side;
          memcpy(param->stats_, picture->stats, sizeof(*param->stats_));
        }
  ...
  if (num_crunch_configs_side != 0) {
    ...
    // The following line is to suppress a Clang warning.
  ...
  if (num_crunch_configs_side != 0) {
    ...
    if (VP8LBitWriterNumBytes(&bw_side) < VP8LBitWriterNumBytes(bw_main)) {
      ...
      if (picture->stats != NULL) {
        memcpy(picture->stats, params_side.stats_, sizeof(*picture->stats));
      }
    }
  } else {
    if (!ok_main) {
