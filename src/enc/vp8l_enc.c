...
#define CONFIGS_CAPACITY_MAX kNumEntropyIx

static int AnalyzeAndInit(VP8LEncoder* const enc,
                          CrunchConfig crunch_configs[CONFIGS_CAPACITY_MAX],
    ...
    if (method == 6 && config->quality == 100) {
      ...
      for (i = 0; i < kNumEntropyIx; ++i) {
        if (i != kPalette || use_palette) {
          ...
          assert(*crunch_configs_size <= CONFIGS_CAPACITY_MAX);
        }
      }
    } else {
...
WebPEncodingError VP8LEncodeStream(const WebPConfig* const config,
  ...
  for (idx = 0; idx < num_crunch_configs; ++idx) {
    ...
    err = EncodeImageInternal(bw, enc->argb_, &enc->hash_chain_, enc->refs_,
                              ...
                              use_cache, crunch_configs[idx].lz77s_types_to_try_,
