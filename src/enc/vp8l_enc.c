...
#define CRUNCH_CONFIGS_SIZE kNumEntropyIx * 2

static int AnalyzeAndInit(VP8LEncoder* const enc,
                          CrunchConfig crunch_configs[CRUNCH_CONFIGS_SIZE],
                          int* const num_crunch_configs,
                          int* const red_and_blue_always_zero) {
  ...
  if (low_effort) {
    ...
    crunch_configs[0].lz77s_ = kLZ77Standard | kLZ77RLE;
    *num_crunch_configs = 1;
  } else {
    ...
    *num_crunch_configs = 0;
    if (method == 6 && config->quality == 100) {
      // Go brute force on all transforms.
      for (i = 0; i < kNumEntropyIx; ++i) {
        if (i != kPalette || use_palette) {
          crunch_configs[*num_crunch_configs].entropy_idx_ = i;
          crunch_configs[(*num_crunch_configs)++].lz77s_ =
              kLZ77Standard | kLZ77RLE;
          assert(*num_crunch_configs <= CRUNCH_CONFIGS_SIZE);
        }
      }
    } else {
      // Only choose the guessed best transform.
      crunch_configs[*num_crunch_configs].entropy_idx_ = min_entropy_ix;
      crunch_configs[(*num_crunch_configs)++].lz77s_ = kLZ77Standard | kLZ77RLE;
    }
  }
...
static WebPEncodingError EncodeImageInternal(
    ...
    int height, int quality, int low_effort, int use_cache, int lz77s,
  ...
  refs_best = VP8LGetBackwardReferences(
      width, height, argb, quality, low_effort, lz77s, cache_bits, hash_chain,
...
WebPEncodingError VP8LEncodeStream(const WebPConfig* const config,
  ...
  for (idx = 0; idx < num_crunch_configs; ++idx) {
    ...
    err = EncodeImageInternal(bw, enc->argb_, &enc->hash_chain_, enc->refs_,
                              ...
                              use_cache, crunch_configs[idx].lz77s_,
