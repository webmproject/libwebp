...
static WebPEncodingError EncodeStreamHook(void* input, void* data2) {
  StreamEncodeContext* params = (StreamEncodeContext*)input;
  ...
  if (num_crunch_configs > 1 && !VP8LBitWriterClone(bw, &bw_best)) {
    err = VP8_ENC_ERROR_OUT_OF_MEMORY;
    goto Error;
  }
...
Error:
  if (num_crunch_configs > 1) VP8LBitWriterWipeOut(&bw_best);
  params->err_ = err;
...
WebPEncodingError VP8LEncodeStream(const WebPConfig* const config,
  ...
  {
    const int params_size = 1 + (num_crunch_configs_side != 0);
  ...
  if (!worker_interface->Sync(&worker_main)) return params_main.err_;
  worker_interface->End(&worker_main);
  // Wait for the second thread if any.
  if (num_crunch_configs_side != 0) {
    if (!worker_interface->Sync(&worker_side)) return params_side.err_;
