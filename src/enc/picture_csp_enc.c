...
static int PictureARGBToYUVA(WebPPicture* picture, WebPEncCSP colorspace,
  ...
  if (picture->argb == NULL) {
    ...
    const uint8_t* const a = argb + (3 ^ ALPHA_OFFSET);
    const uint8_t* const r = argb + (2 ^ ALPHA_OFFSET);
    const uint8_t* const g = argb + (1 ^ ALPHA_OFFSET);
    const uint8_t* const b = argb + (0 ^ ALPHA_OFFSET);
...
static int Import(WebPPicture* const picture,
  ...
  if (import_alpha) {
    uint32_t* dst = picture->argb;   // destination is A,R,G,B for BE, B,G,R,A for LE
    const int do_copy =
#ifdef WORDS_BIGENDIAN
        !swap_rb
#else
