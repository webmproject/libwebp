...
int WebPPlaneDistortion(const uint8_t* src, size_t src_stride,
  ...
  const AccumulateFunc metric = (type == 0) ? AccumulateSSE :
                                (type == 1) ? AccumulateSSIM :
                                              AccumulateLSIM;
  if (src == NULL || src_stride < x_step * width || ref_stride < x_step * width ||
  ...
  if (x_step != 1) {
    ...
    uint8_t* const tmp1 = allocated;
    uint8_t* const tmp2 = tmp1 + width * height;
  ...
  *distortion = metric(src, width, ref, width, width, height);  
  WebPSafeFree(allocated);

  *result = (type == 1) ? (float)GetLogSSIM(*distortion, width * height)
                        : (float)GetPSNR(*distortion, width * height);
  return 1;  
}
