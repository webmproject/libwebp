...
static void NearLossless(int xsize, int ysize, int stride,
                         const uint32_t* argb_src, int limit_bits,
                         uint32_t* tmp_buffer,   // 3-row temporary buffer
                         uint32_t* argb_dst) {
  int x, y;
  const int limit = 1 << limit_bits;
  uint32_t* prev_row = tmp_buffer;
...
int VP8ApplyNearLossless(const WebPPicture* const picture, int quality,
  ...
  uint32_t* const tmp_buffer =
      (uint32_t*)WebPSafeMalloc(xsize * 3, sizeof(*tmp_buffer));
  ...
  NearLossless(xsize, ysize, stride, picture->argb, limit_bits, tmp_buffer,
               argb_dst);
  for (i = limit_bits - 1; i != 0; --i) {
    NearLossless(xsize, ysize, xsize, argb_dst, i, tmp_buffer, argb_dst);
  }
  WebPSafeFree(tmp_buffer);
  return 1;
}
