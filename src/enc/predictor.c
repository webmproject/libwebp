...
static WEBP_INLINE void PredictBatch(int mode, int x_start, int y,
  ...
  if (x_start == 0) {
    if (y == 0) {
      // ARGB_BLACK.
      VP8LPredictorsBatch[0](current, NULL, 1, 0, out);
    } else {
      // Top one.
      VP8LPredictorsBatch[2](current, upper, 1, 0, out);
    }
    ++x_start;
    ++out;
    --num_pixels;
  ...
  if (y == 0) {
    // Left one.
    VP8LPredictorsBatch[1](current + x_start, NULL, num_pixels, 0, out);
  } else {
    VP8LPredictorsBatch[mode](current + x_start, upper + x_start, num_pixels, 0,
                              out);
  }
}
