...
static WEBP_INLINE void PredictBatch(int mode, int x_start, int y,
  ...
  if (x_start == 0) {
    if (y == 0) {
      // ARGB_BLACK.
      VP8LResidualsBatch[0](current, NULL, 1, out);
    } else {
      // Top one.
      VP8LResidualsBatch[2](current, upper, 1, out);
    }
    ++x_start;
    ++out;
    --num_pixels;
  ...
  if (y == 0) {
    // Left one.
    VP8LResidualsBatch[1](current + x_start, NULL, num_pixels, out);
  } else {
    VP8LResidualsBatch[mode](current + x_start, upper + x_start, num_pixels,
                              out);
  }
}
