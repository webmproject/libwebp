...
// In most cases, we don't need to use a full aritmetic decoder, since
// all the header' bits are written using a uniform probability 128.
...
int VP8EstimateQuality(const uint8_t* const data, size_t size) {
  ...
  // base mapping
  Q = (127 - Q) * 100 / 127;
  // correction for power-law behaviour in low range
  if (Q < 80) {
