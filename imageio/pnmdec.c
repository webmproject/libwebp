...
static size_t ReadPAMFields(PNMInfo* const info, size_t off) {
  ...
  while (1) {
    ...
    if (sscanf(out, "WIDTH %d", &tmp) == 1) {
      if (info->have_flags & WIDTH_FLAG) return FlagError("WIDTH");
      info->have_flags |= WIDTH_FLAG;
      info->width = tmp;
    } else if (sscanf(out, "HEIGHT %d", &tmp) == 1) {
      if (info->have_flags & HEIGHT_FLAG) return FlagError("HEIGHT");
      info->have_flags |= HEIGHT_FLAG;
      info->height = tmp;
    } else if (sscanf(out, "DEPTH %d", &tmp) == 1) {
      if (tmp != 4) {
        fprintf(stderr, "unsupported depth value %d\n", tmp);
        return 0;
      }
      if (info->have_flags & DEPTH_FLAG) return FlagError("DEPTH");
      info->have_flags |= DEPTH_FLAG;
      info->depth = tmp;
      // TODO(skal): what to do with DEPTH?
    } else if (sscanf(out, "MAXVAL %d", &tmp) == 1) {
      if (info->have_flags & MAX_VALUE_FLAG) return FlagError("MAX_VALUE");
      info->have_flags |= MAX_VALUE_FLAG;
      info->max_value = tmp;
  ...
  if (!(info->have_flags & TUPLE_FLAG)) {
    info->have_flags |= TUPLE_FLAG;
    info->bytes_per_px = 3;
  }
  if (info->have_flags != ALL_NEEDED_FLAGS) {
...
static size_t ReadHeader(PNMInfo* const info) {
  ...
  if (info->type == 7) {
    ...
    off = ReadLine(info->data, off, info->data_size, out, &out_size);
    if (off == 0 || sscanf(out, "%d %d", &info->width, &info->height) != 2) return 0;
    off = ReadLine(info->data, off, info->data_size, out, &out_size);
...
int ReadPNM(const uint8_t* const data, size_t data_size,
  ...
  if (pixel_bytes > data_size) {
    fprintf(stderr, "Truncated PNM file (P%d).\n", info.type);
    goto End;
  }
  stride = (uint64_t)(info.bytes_per_px < 3 ? 3 : info.bytes_per_px) * info.width;
