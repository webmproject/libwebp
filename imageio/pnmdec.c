...
int ReadPNM(const uint8_t* const data, size_t data_size,
  ...
  int ok = 0;
  int i, j;
  uint64_t stride, pixel_bytes, bytes_per_px, sample_size;
  ...
  bytes_per_px = sample_size * (info.depth < 3 ? 3 : info.depth);
  stride = bytes_per_px * info.width;
  ...
  for (j = 0; j < info.height; ++j) {
    ...
    if (info.max_value < 255) {
      const uint32_t round = info.max_value / 2;
      for (i = 0; i < info.width * (int)bytes_per_px; ++i) {
        tmp_rgb[i] = (tmp_rgb[i] * 255 + round) / info.max_value;
      }
    }
