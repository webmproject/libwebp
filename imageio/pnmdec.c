...
static size_t ReadPNMHeader(const uint8_t* const data, size_t data_size,
...
int ReadPNM(const uint8_t* const data, size_t data_size,
  ...
  size_t offset = ReadPNMHeader(data, data_size, &width, &height,
                                &type, &max_value);
  ...
  if (width <= 0 || height <= 0) {
    fprintf(stderr, "Invalid P%d PNM resolution (%dx%d).\n",
            type, width, height);
    goto End;
  }
  if (max_value <= 0 || max_value > 255) {
    fprintf(stderr, "Invalid / unsupported max pixel value: %d.\n", max_value);
    goto End;
  }
