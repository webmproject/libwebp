...
static HRESULT CreateOutputStream(const GCHAR* out_file_name,
  ...
  if (write_to_mem) {
    ...
    IFS(SHCreateStreamOnFile(out_file_name, STGM_WRITE | STGM_CREATE, stream));
  }
  if (FAILED(hr)) {
...
static HRESULT WriteUsingWIC(const GCHAR* out_file_name, int use_stdout,
...
int WebPWritePNG(const char* out_file_name, int use_stdout,
  ...
  return SUCCEEDED(WriteUsingWIC((const GCHAR*)out_file_name, use_stdout,
...
int WebPSaveImage(const WebPDecBuffer* const buffer,
  ...
  const GCHAR* const out_file_name = (const GCHAR*)file_name;
  FILE* fout = NULL;
