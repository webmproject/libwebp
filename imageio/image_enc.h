...
// Save YUV or YUVA samples as PGM format (using IMC4 layout).
int WebPWriteYUV(FILE* fout, const struct WebPDecBuffer* const buffer);

// Save raw YUV samples (RAW_YUV)
int WebPWriteRawYUV(FILE* fout, const struct WebPDecBuffer* const buffer);

// Save 16b mode (RGBA4444, RGB565, ...) as PGM format, for debugging purpose.
int WebPWrite16bAsPGM(FILE* fout, const struct WebPDecBuffer* const buffer);
