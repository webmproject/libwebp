...
static HRESULT OpenInputStream(const wchar_t* filename, IStream** stream) {
  HRESULT hr = S_OK;
  if (!wcscmp(filename, L"-")) {
    ...
    IFS(SHCreateStreamOnFileW(filename, STGM_READ, stream));
  }

  if (FAILED(hr)) {
    fwprintf(stderr, L"Error opening input file %s (%08lx)\n", filename, hr);
  }
  return hr;
}
...
int ReadPictureWithWIC(const char* const filename,
  ...
  IFS(OpenInputStream((const wchar_t*)filename, &stream));
