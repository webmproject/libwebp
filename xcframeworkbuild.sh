...
if [[ -z "${XCODE}" ]] || [[ "${XCODE%%.*}" -lt 12 ]]; then
  echo "Xcode 12.0 or above is required!"
  exit 1
fi
...
PLATFORMS[$MACOS]="MacOSX-x86_64 MacOSX-arm64"
PLATFORMS[$MACOS_CATALYST]="MacOSX-Catalyst-x86_64 MacOSX-Catalyst-arm64"
readonly PLATFORMS
...
if [[ -z "${SDK[$IOS]}" ]] || [[ ${SDK[$IOS]%%.*} -lt 8 ]]; then
  echo "iOS SDK version 8.0 or above is required!"
  exit 1
fi
